...
# ================================================
# TF-IDF / RARE WORD SETTINGS
# ================================================
# These control the additional rare-word analyses.

# How many top features to use from the TF-IDF weighted table.
# A larger window captures more rare vocabulary signal.
TFIDF_MFW_MIN <- 14000
TFIDF_MFW_MAX <- 18000
TFIDF_MFW_INCR <- 1000

# Minimum number of texts a word must appear in to be kept.
# Set to 2 to exclude words unique to a single text (likely
# proper nouns, OCR errors, etc.) while still keeping rare words.
TFIDF_MIN_DOCS <- 2
...
# ================================================
# BUILD TF-IDF WEIGHTED FREQUENCY TABLE
# ================================================

cat("========================================\n")
cat("COMPUTING TF-IDF WEIGHTED TABLE\n")
cat("========================================\n")

# Step 1: Filter out words that appear in fewer than TFIDF_MIN_DOCS texts.
#          This removes hapax legomena and OCR artefacts while keeping
#          genuinely rare vocabulary shared across at least two texts.
doc_counts <- colSums(freq_table > 0)
keep_cols  <- doc_counts >= TFIDF_MIN_DOCS
freq_filtered <- freq_table[, keep_cols, drop = FALSE]

cat("Words before filtering:", ncol(freq_table), "\n")
cat("Words after min-doc filter (>=", TFIDF_MIN_DOCS, "texts):", ncol(freq_filtered), "\n")

# Step 2: Compute IDF weights.
#          IDF = log(N / df) where N = number of texts, df = number of texts
#          containing the word.  Rare words get high IDF; ubiquitous words ≈ 0.
n_docs <- nrow(freq_filtered)
df     <- colSums(freq_filtered > 0)
idf    <- log(n_docs / df)

# Step 3: Multiply each text's relative frequency by the IDF weight.
tfidf_table <- sweep(freq_filtered, 2, idf, "*")

# Step 4: Re-sort columns so that the highest-mean-TF-IDF words come first.
#          This way stylo's MFW window selects the most *distinctively used*
#          words rather than the most frequent ones.
col_order   <- order(colMeans(tfidf_table), decreasing = TRUE)
tfidf_table <- tfidf_table[, col_order, drop = FALSE]

cat("TF-IDF table ready:", nrow(tfidf_table), "texts ×",
    ncol(tfidf_table), "features\n")
cat("Top 20 TF-IDF ranked words:\n")
cat(" ", paste(head(colnames(tfidf_table), 20), collapse = ", "), "\n")
cat("========================================\n\n")

# Save the TF-IDF word ranking for inspection
tfidf_ranking <- data.frame(
  rank       = seq_len(ncol(tfidf_table)),
  word       = colnames(tfidf_table),
  mean_tfidf = colMeans(tfidf_table),
  doc_freq   = colSums(freq_filtered[, col_order, drop = FALSE] > 0),
  stringsAsFactors = FALSE
)
write.csv(tfidf_ranking, file = "tfidf_word_ranking.csv", row.names = FALSE)
cat("Saved TF-IDF word ranking to tfidf_word_ranking.csv\n\n")
...
# ================================================
# STEP 3: TF-IDF WEIGHTED ANALYSES (RARE WORDS)
# ================================================
# These repeat the same analysis types and distance
# measures but use the TF-IDF weighted frequency table
# so that rare, distinctive vocabulary drives clustering.

cat("\n########################################\n")
cat("### STEP 3: TF-IDF WEIGHTED ANALYSES\n")
cat("### (Rare / distinctive word signal)\n")
cat("########################################\n\n")

# --- 3a: PCV on TF-IDF table ---

cat("Processing: TF-IDF PCV\n")

filename <- paste0("TFIDF_", feature_type, "_", TFIDF_MFW_MIN, "-", TFIDF_MFW_MAX,
                   "_PCV_", timestamp)

results <- stylo(gui = FALSE,
                 frequencies = tfidf_table,
                 analyzed.features = ANALYZED_FEATURES,
                 ngram.size = NGRAM_SIZE,
                 preserve.case = PRESERVE_CASE,
                 distance.measure = "delta",
                 analysis.type = "PCV",
                 encoding = ENCODING,
                 mfw.min = TFIDF_MFW_MAX,
                 mfw.max = TFIDF_MFW_MAX,
                 mfw.incr = 0,
                 culling.min = 0,
                 culling.max = 0,
                 culling.incr = 0,
                 mfw.list.cutoff = ncol(tfidf_table),
                 consensus.strength = CONSENSUS_STRENGTH,
                 sampling = SAMPLING,
                 sample.size = SAMPLE_SIZE,
                 number.of.samples = NUMBER_OF_SAMPLES,
                 ...
                 plot.line.thickness = PLOT_LINE_THICKNESS,
                 plot.options.reset = PLOT_OPTIONS_RESET,
                 custom.graph.title = "TF-IDF PCV (Rare Word Signal)",
                 custom.graph.filename = filename,
                 text.id.on.graphs = TEXT_ID_ON_GRAPHS,
                 colors.on.graphs = COLORS_ON_GRAPHS,
                 titles.on.graphs = TITLES_ON_GRAPHS,
...
cat("Completed: TF-IDF PCV\n")

# --- 3b: CA, BCT, MDS on TF-IDF table for each distance measure ---

for (dist_measure in distance_measures) {
  
  cat("\n========================================\n")
  cat("TF-IDF DISTANCE MEASURE:", dist_measure, "\n")
  cat("========================================\n")
  
  for (analysis_type in c("CA", "BCT", "MDS")) {
    
    cat("\nProcessing: TF-IDF", analysis_type, "with", dist_measure, "\n")
    
    filename <- paste0("TFIDF_", feature_type, "_", TFIDF_MFW_MIN, "-", TFIDF_MFW_MAX,
                       "_", dist_measure, "_", analysis_type, "_", timestamp)
    
    use_mfw_min  <- if(analysis_type == "BCT") TFIDF_MFW_MIN else TFIDF_MFW_MAX
    use_mfw_incr <- if(analysis_type == "BCT") TFIDF_MFW_INCR else 0
    ...
    results <- stylo(gui = FALSE,
                     ...
                     plot.line.thickness = PLOT_LINE_THICKNESS,
                     plot.options.reset = PLOT_OPTIONS_RESET,
                     custom.graph.title = paste("TF-IDF", analysis_type,
    ...
    cat("Completed: TF-IDF", analysis_type, "with", dist_measure, "\n")
  }
  
  cat("\n========================================\n")
  cat("Completed all TF-IDF analysis types for", dist_measure, "\n")
  cat("========================================\n")
}
...
cat("========================================\n")
cat("Standard MFW PDFs created: 31\n")
cat("  - PCV: 1\n")
cat("  - CA/BCT/MDS: 30 (3 types × 10 distances)\n")
cat("TF-IDF PDFs created: 31\n")
cat("  - TF-IDF PCV: 1\n")
cat("  - TF-IDF CA/BCT/MDS: 30 (3 types × 10 distances)\n")
cat("TOTAL PDFs: 62\n")
cat("========================================\n")
...
cat("\n--- TF-IDF (Rare Words) ---\n")
cat("TF-IDF MFW range:", TFIDF_MFW_MIN, "-", TFIDF_MFW_MAX, "by", TFIDF_MFW_INCR, "\n")
cat("Min docs per word:", TFIDF_MIN_DOCS, "\n")
cat("Words kept after filtering:", ncol(tfidf_table), "\n")
cat("\n--- Shared ---\n")
...
cat("  ✓ Corpus loaded once (not 62 times)\n")
cat("  ✓ TF-IDF table computed once and reused\n")
...
tfidf_pdfs  <- grep("^TFIDF_", pdf_files, value = TRUE)
standard_pdfs <- setdiff(pdf_files, tfidf_pdfs)

cat("Standard PDF graphs:", length(standard_pdfs), "\n")
cat("TF-IDF PDF graphs:", length(tfidf_pdfs), "\n")
