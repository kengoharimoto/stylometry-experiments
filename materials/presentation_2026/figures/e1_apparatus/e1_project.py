...
Usage: e1_project.py w|c [B] [--noreuse]

--noreuse (2026-08-19, reframe R3): project the apparatus/augmented units
into the fixed no-reuse map (manifest noreuse2026_n126, orientation
mds3d/coords_C3-500ns_noreuse_n126.tsv). C3 only — see R1. NOTE the
asymmetry: the map is reuse-stripped, the projected apparatus files are
not (the strip was computed over the base corpus, and the CE apparatus is
external to it). The test is directional — does known-later material still
measure later — so the asymmetry weakens rather than manufactures the
result; state it wherever these numbers are printed.
"""
import csv
import re
import sys
...
if NOREUSE:
    ...
    GEN_CORPUS = ROOT / 'corpus/e1_apparatus_sandhied'
